...
for t=1:179
    ...
    for n=1:nmax
        ...
        Q3a(n)=Q*cos(phiQ)*(sqrt(2*n+1))*(sqrt(1/(4*pi)))*(rQ^(n-1)*(a02(n)*n)-rQ^(-1*(n+2))*(b02(n)*(n+1)))/((2*n+1)*sr2*(a02(n)*b12(n)-a12(n)*b02(n)));
        
        
        yya(n)=(Q3a(n)*(b13(n)+((r2^(2*n+1))*a13(n)))*sqrt((2*n+1)/(4*pi))*((n+1)*cos(teta)*pq1(n)-n*pq2(n)))/((2*n+1)*rr^(n+1));
        vphi_a=vphi_a+yya(n);

        
    end
    Bphi_a=(vphi_a)/sin(teta);
...
for s=1:smax
    for j=1:s
        G(s,j)=((R1^(2*j+1)).*C1a(j))./(C1b(j).*(a^(j+1)).*(factorial(j-1)).*(factorial(s-j)));
    end
    zy(s)=((-1)*(s.^2+s).*(m11(s)+m12(s)))./(((factorial(s+1)).^2).*(m21(s)+m22(s)).*(Roh0^(2*s+1)));
    for j=s:smax
        H(s,j)=zy(s).*(((-1)^(j-s)).*factorial(j).*(a^j))./(factorial(j-s));
    end
end            
P=G+H;
...
for t=1:179
    ...
    for s=1:smax
        zz(s)=((s*f^(s-1))+((R1^(2*s+1))*ZT(s)))/(((k1+1)*s+1)*rr^(s)*sin(teta));
...
legend('Anisotropic model','Isotropic model')