...
for t=1:179
    teta=t*pi/180;
    a=sqrt(ro^2+rQ^2-(2*ro*rQ*cos(teta)));
    F=a*((a*ro)+ro^2-(ro*rQ*cos(teta)));
    gradFr=(((a^2/ro)+((ro*(ro-rQ*cos(teta)))/a)+(2*a)+(2*ro))*ro)-((a+(2*ro)+(ro*(ro-rQ*cos(teta))/a))*(rQ*cos(teta)));
    gradFteta=(a+(2*ro)+(ro*(ro-rQ*cos(teta))/a))*(rQ*sin(teta));
    Br=(zz*sin(teta)*sin(phi)*((ro*gradFr)-F))/(F^2);
    Bteta=(zz*sin(phi)*((ro*sin(teta)*gradFteta)-(F*cos(teta))))/(F^2);
    Bphi=(((-1)*zz)*(F*cos(phi)))/(F^2);
    Bx(t)=sqrt((Br^2)+(Bteta^2)+(Bphi^2));
end
m=1:179;
plot(m,Bx(m))
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
legend('Anisotropic Eccentric model','Anisotropic Cocentric model')
